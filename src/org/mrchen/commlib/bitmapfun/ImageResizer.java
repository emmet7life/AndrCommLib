 ...
 * 一个继承自ImageWorker的子类，该类用来重新计算资源图片到一个目标宽高，避免当图片资源很大时直接导入到内存中，这是非常有用的
 */
public class ImageResizer extends ImageWorker {
	...
	/**
	 * Set the target image width and height. 设置目标图的宽和高
	 * 
	 * @param width
	 * @param height
	 */
	 ...
	 * 
	 * @param filename
	 *            The full path of the file to decode 全路径名称
	 * @param reqWidth
	 *            The requested width of the resulting bitmap
	 * @param reqHeight
	 *            The requested height of the resulting bitmap
	...
	public static int calculateInSampleSize(BitmapFactory.Options options, int reqWidth, int reqHeight) {
		...
		if (height > reqHeight || width > reqWidth) {
			...
			// be more aggressive侵略性，气势汹汹的 with sample down the image (=larger
			// inSampleSize).
			// 这里提供一个额外逻辑，假如有这种情况，该图资源有一个很大的宽高比，比如一张全景图相对它的高来说，宽度或许是非常大的，
			// 这种情况下，图的总像素依然很大，很难积极的适应内存开销，所以我们应该再大幅度得调整inSampleSize
