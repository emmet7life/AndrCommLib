...
package org.mrchen.commlib.bitmapfun;
...
import org.mrchen.commlib.BuildConfig;
...
public class ImageCache {
	...
	// Default memory cache size 默认的内存缓存空间
	private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 1024 * 5; // 5MB

	// Default disk cache size 默认的存储卡缓存空间
	private static final int DEFAULT_DISK_CACHE_SIZE = 1024 * 1024 * 10; // 10MB

	// Compression settings when writing images to disk cache 存储到存储卡时的图片压缩格式
	...
	private static final boolean DEFAULT_MEM_CACHE_ENABLED = true;// 缓存内存开关
	private static final boolean DEFAULT_DISK_CACHE_ENABLED = true;// 缓存存储卡开关
	private static final boolean DEFAULT_CLEAR_DISK_CACHE_ON_START = false;// 启动时清除存储卡缓存开关
	private static final boolean DEFAULT_INIT_DISK_CACHE_ON_CREATE = false;// 创建时初始化存储卡缓存开关

	private DiskLruCache mDiskLruCache;// 存储卡缓存
	private LruCache<String, Bitmap> mMemoryCache;// 内存缓存
	private ImageCacheParams mCacheParams;// 图片缓存参数配置类
	private final Object mDiskCacheLock = new Object();// 锁
	private boolean mDiskCacheStarting = true;// 标记
	...
	private void init(ImageCacheParams cacheParams) {
		...
		if (mCacheParams.memoryCacheEnabled) {
			...
			mMemoryCache = new LruCache<String, Bitmap>(mCacheParams.memCacheSize) {
				 ...
				 * practical(实用的) for a bitmap cache
				 */
				@Override
		...
		// initialized
		// on a separate(另外的) thread due to disk access.
	...
	public Bitmap getBitmapFromDiskCache(String data) {
		...
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
					// 当初始化完成时会设置mDiskCacheStarting为false并调用mDiskCacheLock.notifyAll()解锁
					mDiskCacheLock.wait();
	 ...
	 * (清除与该ImageCache对象相关的内存和存储卡缓存，注意：不可在主线程即UI线程执行该代码，因为涉及到磁盘操作)
	 */
	...
	public static class RetainFragment extends Fragment {
		...
		/**
		 * Empty constructor as per the Fragment documentation<br>
		 * 当系统从保存的状态还原Fragment时，会调用默认的构造函数，然后附件初始化参数包
		 */
		public RetainFragment() {
		}

		// 在onAttach调用之前调用，通常是在活动的setContentView中传入由xml布局文件定义的View，其中使用了标签<fragment>定义的片段。
		// 如果重新创建碎片，并且之前在onSaveInstanceState()中保存了某种状态，此包（Bundle）将包含保存的状态值。
		// （备注：onInflate()文档与实际使用不符，文档表明onInflate()始终在onAttach()之前调用。实际上，在Activity重新启动后，
		// onInflate()可能在onCreateView()之后调用。这对于将值设置到包（Bundle）中并调用setArguments()而言太迟了，
		// Android官方文档中Fragment生命周期的图示中也没有将onInflate()包含在生命周期，看来Android的大牛们也很难预测onInflate()会在何时回调）
		@Override
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			// 当在onCreate()方法中调用了setRetainInstance(true)后，Fragment恢复时会跳过
			// onCreate()和onDestroy()方法，因此不能在onCreate()中放置一些初始化逻辑，切忌！
