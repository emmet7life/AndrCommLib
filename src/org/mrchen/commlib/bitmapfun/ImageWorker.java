...
/**
 * This class wraps封装 up completing some arbitrary任意 long running work when
 * loading a bitmap to an ImageView. It handles things like using a memory and
 * disk cache, running the work in a background thread and setting a placeholder
 * image.该类封装了完成任意长时间任务比如加载一个位图到ImageView控件中，它可以除以这些事，比如使用内存或者存储卡缓存，
 * 在后台线程中允许该任务并且设置一个临时显示或者说是默认的图片
 */
public abstract class ImageWorker {
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			// 1.首先从内存中获取可用的缓存Bitmap数据
			// If the image cache is available and this task has not been
			// cancelled by another
			// thread and the ImageView that was originally bound to(绑定) this
			// task
			// is still bound back
			...
			// 2.如果缓存未找到并且任务未被取消掉则由传入的params[0]从实现类中获取Bitmap，具体实现类怎么获取则各自定义
			...
			// 3.如果该Bitmap在实现类中被处理了(即返回了)并且缓存类有效，这时我们将这个被返回了的Bitmap
			// 添加到缓存中以便将来使用，注意，我们这里没有检查任务是否被取消，就算是该任务被取消了，并且该线程仍然
			// 在运行，我们或许仍然应该将Bitmap缓存起来因为或许在将来的某个时刻我们将有可能重新使用到它
		...
		/**
		 * Returns the ImageView associated with this task as long as(只要) the
	 ...
	 * on the ImageView. 当处理过程完毕并且最终得到的Bitmap对象需要被设置给ImageView时调用该方法
	 * 
	 * @param imageView
	 * @param bitmap
	 */
	private void setImageBitmap(ImageView imageView, Bitmap bitmap) {
		if (mFadeInBitmap) {
			...
			// bitmap
			// 构建一个过渡的Drawable，由new ColorDrawable(android.R.color.transparent)
			// -> new BitmapDrawable(mResources, bitmap)
			final TransitionDrawable td = new TransitionDrawable(new Drawable[] {
					new ColorDrawable(android.R.color.transparent), new BitmapDrawable(mResources, bitmap) });
			// Set background to loading bitmap TODO 这里为啥要设置背景？不解
