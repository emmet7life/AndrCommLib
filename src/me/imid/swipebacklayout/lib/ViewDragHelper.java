...
public class ViewDragHelper {
	...
	private void dragTo(int left, int top, int dx, int dy) {
		...
		if (dx != 0) {
			...
			int offset = clampedX - oldLeft;
			mCapturedView.offsetLeftAndRight(offset);
			// LogHelper.i(TAG, "X >> oldTop:" + oldTop + ", clampedX:" +
			// clampedX + ", offset:" + offset + ", View ID:"
			// + mCapturedView.getId());
		}
		if (dy != 0) {
			clampedY = mCallback.clampViewPositionVertical(mCapturedView, top, dy);
			int offset = clampedY - oldTop;
			mCapturedView.offsetTopAndBottom(offset);
			// LogHelper.i(TAG, "Y >> oldTop:" + oldTop + ", clampedY:" +
			// clampedY + ", offset:" + offset + ", View ID:"
			// + mCapturedView.getId());
		}
