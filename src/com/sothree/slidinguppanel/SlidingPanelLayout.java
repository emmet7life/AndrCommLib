...
public class SlidingPanelLayout extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		int start = 0;
		int nextStart = 0;
		if (isPanelOnTop() || isPanelOnBottom()) {
			start = paddingTop;
			nextStart = start;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			boolean goon = true;
			// 判断触摸区
			if (mSlideableView != null && mSlideableView.getVisibility() == View.VISIBLE) {
				int t = mSlideableView.getTop();
				int b = mSlideableView.getBottom();
				LogHelper.d(TAG, "onInterceptTouchEvent >> mSlideableView t : " + t + ", b : " + b);
				Rect temp = new Rect();
				if (isPanelOnBottom()) {
					temp.right = mSlideableView.getRight();
					temp.bottom = t;
					if (temp.contains((int) x, (int) y)) {
						goon = false;
						collapsePane();
						interceptTap = false;
					}
				} else if (isPanelOnTop()) {
					temp.right = mSlideableView.getRight();
					temp.top = b;
					temp.bottom = getMeasuredHeight();
					if (temp.contains((int) x, (int) y)) {
						goon = false;
						collapsePane();
						interceptTap = false;
			...
			//
			if (goon && mDragViewHit && !mIsUsingDragViewTouchEvents) {
				interceptTap = true;
			}
			break;
		}
	...
	private void onPanelDragged(int newTop) {
		if (isPanelOnBottom()) {
			...
			if (newTop < 0) {
				...
				int distance = Math.abs(topBound) - Math.abs(newTop);
				mSlideOffset = 1 - (float) distance / mSlideRange;
			} else {
				mSlideOffset = (float) (getPaddingTop() - newTop) / mSlideRange;
			}
			Log.d(TAG, "onPanelDragged >> newTop:" + newTop + ", mSlideOffset:" + mSlideOffset);
		}
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		if (drawScrim) {
			...
			final int imag = (int) (baseAlpha * (1 - offset));
	...
	boolean smoothSlideTo(float slideOffset, int velocity) {
		...
		if (isPanelOnBottom()) {
			y = (int) (paddingTop + slideOffset * mSlideRange);
		} else if (isPanelOnTop()) {
			// 控件本身的高度
			final int layoutHeight = getMeasuredHeight() - paddingTop - paddingBottom;
			// 对应slideOffset下，滑动控件的bottom布局位置
			final int bottomBound = (int) (1 - slideOffset) * mSlideRange + mPanelHeight + paddingTop;
			// 确定最终的位置
			y = bottomBound - layoutHeight;
		}
	...
	private boolean isPanelOnLeft() {
		return mPanelOnDirection == PANEL_ON_LEFT;
	}

	private boolean isPanelOnRight() {
		return mPanelOnDirection == PANEL_ON_RIGHT;
	}

	private boolean isPanelOnTop() {
		return mPanelOnDirection == PANEL_ON_TOP;
	}

	private boolean isPanelOnBottom() {
		return mPanelOnDirection == PANEL_ON_BOTTOM;
	}
				...
				if (mAnchorPoint != 0) {
					...
					if (yvel > 0 || (yvel == 0 && mSlideOffset >= (1f + anchorOffset) / 2)) {
						...
						if (isPanelOnBottom())
							...
							finalTop = (int) (bottomBound + mSlideRange * (1 - mAnchorPoint)) - layoutHeight;
					}
